...
const fields=['prompt','durationS','ratio','resolution','generateAudio','firstFrameReferenceId','lastFrameReferenceId','referenceImageIds','referenceVideoIds'];
export const taskApiVideoInputSchema:JsonObject={type:'object',additionalProperties:false,required:['prompt'],properties:{prompt:{type:'string',minLength:1},durationS:{type:'number',exclusiveMinimum:0},ratio:{type:'string'},resolution:{type:'string'},generateAudio:{type:'boolean'},firstFrameReferenceId:{type:'string'},lastFrameReferenceId:{type:'string'},referenceImageIds:{type:'array',items:{type:'string'}},referenceVideoIds:{type:'array',items:{type:'string'}}}};
function validate(input:JsonObject):Support {
  ...
  for(const name of ['referenceImageIds','referenceVideoIds'])if(input[name]!==undefined&&(!Array.isArray(input[name])||(input[name] as unknown[]).some(v=>typeof v!=='string'||!v)))return {supported:false,reason:`${name} must contain reference IDs`};
...
export const taskApiProvider:ProviderDefinition={
  ...
  configSchema:{type:'object',required:['baseUrl'],properties:{baseUrl:{type:'string'},models:{type:'array',items:{type:'string'}}},additionalProperties:false},
  create(context){
    const config=context.config;if(Object.keys(config).some(k=>!['baseUrl','models'].includes(k)))throw Error('Unsupported task-api configuration');
    ...
    const base=config.baseUrl.replace(/\/+$/,''),models=config.models??[];
    if(!Array.isArray(models)||models.some(m=>typeof m!=='string'||!m))throw Error('models must contain model IDs');
    const check=(request:ProviderRequest):Support=>request.capability!=='video.generate'||!request.model?{supported:false,reason:'videoGen: model is required'}:models.length&&!models.includes(request.model)?{supported:false,reason:'Model is not configured on this channel'}:validate(request.input);
    ...
    const compile=async(input:JsonObject,call:ProviderCall)=>{
      const params:JsonObject={prompt:input.prompt};
      ...
      for(const [name,field,kind,array] of [['firstFrameReferenceId','first_frame_image','image',false],['lastFrameReferenceId','last_frame_image','image',false],['referenceImageIds','reference_images','image',true],['referenceVideoIds','reference_videos','video',true]] as const){
    ...
    return {
      models:()=>models.map(id=>defineModel({id:id as string,capability:'video.generate',inputSchema:taskApiVideoInputSchema,outputKinds:['video'],validate})),
      supports:check,
      async submit(request,call){
        ...
        const response=await context.fetch(`${base}/api/v1/tasks`,{method:'POST',headers:auth,body:JSON.stringify({model:request.model,params}),signal:call.signal});
        const body=await response.json().catch(()=>({})) as {task_id?:unknown;detail?:{message?:string}};
      ...
      async poll(receipt,call){
        ...
        const body=await response.json().catch(()=>({})) as {status?:string;detail?:{message?:string};error_message?:string;cost?:number;result?:{video_url?:string;duration?:number;resolution?:string;ratio?:string}};
        ...
        if(body.status==='failed')return {state:'failed',error:{phase:'poll',code:'TASK_API_FAILED',message:body.error_message??'Provider generation failed',outcome:'rejected',retryable:false}};
...
export class TaskApiVideoGen implements VideoGenerator {
  ...
  constructor(config:VideoGenEndpointConfig){
    ...
    this.delegate=asVideoGenerator(registry.create({id:'legacy-task-api',provider:'task-api',config:{baseUrl:config.baseUrl},credentials:{apiKey:{store:'host',key:'task-api'}}},{resolveCredential:async()=>config.apiKey}));
  }
