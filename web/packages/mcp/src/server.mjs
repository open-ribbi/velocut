...
export async function createVelocutServer(options = {}) {
  ...
  register('velocut_scene_edit', 'One validated, compiled, undoable transaction. Operations: add/update/remove/duplicate/duplicateMany/transform/layout/array/assembly/scene. dryRun validates and compiles without committing; returns createdIds and copy ID maps. Read scene_assets for grammar and pass expectedRevision from inspection.',
    { ...asset, edits: z.array(jsonObject).min(1).max(500), expectedRevision, dryRun: z.boolean().optional() }, relay('sceneEdit'));
  ...
  register('velocut_script', 'Run JavaScript in Velocut’s isolated, network-free editor sandbox (top-level await, return JSON). Pure helpers: ops.<commandType>(args), ref(operationId,field). RPC methods: capabilities, query, transaction, document, evaluate, seek, apply, sceneAssets, sceneClip, sceneEdit, sceneArrange, sceneInspect, sceneImportModel, directorSession, previewSession, observe (numbers only). Use loops to generate geometry or batches. Check result.ok; use the separate observe tool for images.',
