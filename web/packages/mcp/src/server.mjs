...
export async function createVelocutServer(options = {}) {
  ...
  register('velocut_scene_edit', 'One validated, compiled, undoable transaction. Operations include geometry.create/geometry.update/geometry.remove/makeUnique plus add/update/remove/duplicate/duplicateMany/transform/layout/array/assembly/scene. preflight:true checks structure and budgets without GPU work; dryRun:true also compiles without committing. includeSpec:false keeps replies compact. Shared instances use add kind:prop model:prop/instance geometryId. Read scene_assets for grammar and pass expectedRevision.',
