...
export interface GenerationReference {id:string;name:string;src:string;createdAt:number;provenance:Record<string,unknown>}
...
export class GenerationManager {
  ...
  private plan(slot:GenerationSlot){
    const request=GenerationRequestSchema.parse(slot.request),channel=this.adapter.channels().find(c=>c.id===request.channel);
  ...
  async execute(input:unknown,options:{dispatch?:(c:Command)=>Envelope;signal?:AbortSignal}={}){
    try{
      ...
      if(p.action==='captureReference'){
        ...
        const refId='ref_'+crypto.randomUUID(),src=await this.adapter.saveReference(refId,captured.blob),reference={id:refId,src,name:captured.name,createdAt:Date.now(),provenance:{...captured.provenance,revision}};
  ...
  private async process(jobId:string){
    ...
    try{
      ...
      if(await this.adapter.binding(job.request.channel)!==job.binding)throw Error('Channel endpoint or credentials changed; restore its configuration before resuming');
      if(job.state==='queued'){
        job=await this.update(jobId,j=>{if(j.state!=='queued')throw Error('Generation state changed');j.state='preparing';delete j.error;});
        const ref=(await this.read()).references.find(r=>r.id===job.request.firstFrameReferenceId);
        const referenceUrl=ref?await this.adapter.prepareReference(ref,controller.signal):undefined;
        controller.signal.throwIfAborted();
        if(await this.adapter.binding(job.request.channel)!==job.binding)throw Error('Channel endpoint or credentials changed during preparation');
