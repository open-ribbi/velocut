...
import type { ObserveResult } from './results';
  ...
  return (
    fg.layers
      .map((l) => {
        ...
        if (l.transition)
          s += ` ←transition ${l.transition.kind}(from ${l.transition.from?.clipId})`;
        return s;
      })
      .join(', ') || '(empty frame)'
  );
...
export async function observeForAgent(
  ...
  if (input.mode === 'scene') {
    const assetId =
      ...
      doc.tracks.flatMap((t) => t.clips).find((c) => c.id === input.source?.clipId)?.assetId;
    if (!assetId)
      return {
        ok: false,
        summary: 'scene observation requires source.assetId or source.clipId',
        images: [],
      };
    const r = await inspectScene(
      store,
      {
        assetId,
        timeS: (input.at ?? 0) / 1e6,
        view: input.view,
        camera: input.camera,
    ...
    return {
      ok: true,
      summary: `Scene ${assetId}, revision ${r.revision}, ${r.timeS}s, ${input.view ?? 'perspective'} view`,
      images,
      data: {
        assetId,
        revision: r.revision,
  ...
  if (!doc.tracks.length && !doc.assets.length)
    return { ok: false, summary: 'The project is empty; there is nothing to observe.', images: [] };
  ...
  const target =
    res === 'full' ? Math.min(TARGET.full, Math.max(doc.width, doc.height)) : TARGET[res];
  ...
  if (mode === 'frame') {
    ...
    if (!grab)
      return {
        ok: false,
        summary: `${head}: nothing renderable (not decoded yet, or out of range).`,
        images: [],
      };
    const audio = await observer.audio(audioRefs, at);
    const images = input.metricsOnly
      ? []
      : [{ base64: await observer.toJpeg(grab.bitmap), mediaType: JPEG }];
    grab.bitmap.close();
    return {
      ok: true,
  ...
  if (mode === 'contact') {
    ...
    for (const p of points) {
      const spec: GrabSpec = aid
        ? { kind: 'asset', assetId: aid, sourceTimeUs: p.t }
        : { kind: 'graph', fg: store.evaluate(p.t) };
      ...
      index.push({
        ...
        sharpness: g.metrics.sharpness,
      });
    }
    if (!cells.length)
      return {
        ok: false,
        summary: 'No renderable frames (assets may not be decoded yet).',
        images: [],
      };
    const cols = Math.min(cells.length, Math.ceil(Math.sqrt(cells.length)));
    const sheet = await observer.toSheet(
      cells,
      cols,
      cells[0].bitmap.width,
      cells[0].bitmap.height,
    );
  ...
  if (mode === 'audio') {
    ...
    if (!a)
      return {
        ok: false,
        summary:
          'No audio asset to analyze (pass {assetId} in source, or import an asset with audio first).',
        images: [],
      };
    ...
    const sil = r.silences.map(
      (s) => `${(s.startUs / 1e6).toFixed(2)}–${(s.endUs / 1e6).toFixed(2)}`,
    );
  ...
  if (mode === 'shots') {
    ...
    if (!target)
      return {
        ok: false,
        summary:
          'No video asset to analyze (pass {assetId}/{clipId} in source, or import a video first).',
        images: [],
      };
    ...
    if (!r.shots.length)
      return {
        ok: false,
        summary: `Asset ${target} produced no decodable frames; cannot segment shots.`,
        images: [],
        data: r,
      };
  ...
  const from = input.from ?? 0;
  const to =
    input.to ?? (aid ? (doc.assets.find((a) => a.id === aid)?.durationUs ?? 0) : docDuration(doc));
  ...
  for (let i = 0; i < n; i++) {
    ...
    const sceneScore =
      prev && m
        ? Math.round(
            Math.abs(m.luma - prev.luma) +
              colorDist(m.avgColor, prev.avgColor) +
              Math.abs(m.colorfulness - prev.colorfulness),
          )
        : 0;
    if (m) prev = m;
    windows.push({
  ...
  const loud = [...windows]
    .sort((a, b) => b.loudnessDbfs - a.loudnessDbfs)
    .slice(0, 3)
    .map((w) => +(w.atUs / 1e6).toFixed(1));
  return {
    ok: true,
