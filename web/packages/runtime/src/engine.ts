...
export interface EngineInit {
  ...
  fpsNum: number;
  fpsDen: number;
  /** Optional public URL prefix for the Rust/WASM artifacts. */
  wasmBase?: string;
}
...
export async function createEngine(init: EngineInit): Promise<ICoreEngine> {
  try {
    ...
    const resp = await fetch(`${(init.wasmBase ?? '/wasm').replace(/\/$/, '')}/velocut_wasm.js`);
    ...
    await mod.default({
      module_or_path: `${(init.wasmBase ?? '/wasm').replace(/\/$/, '')}/velocut_wasm_bg.wasm`,
    });
    ...
    const probe=JSON.parse(inner.apply(JSON.stringify({type:'addGenerationSlot',trackId:'__feature_probe__',startUs:0,durationUs:1,request:{channel:'',model:'',prompt:'',parameters:{},referenceVideoIds:[],input:{camera:{moves:['pan']}},modelRevision:'probe'}})));
