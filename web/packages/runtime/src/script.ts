...
import type { ScriptResult } from './results';
...
export interface ScriptApi {
  ...
  sceneSpatial(opts: unknown): Promise<unknown>;
  ...
  return new Promise<ScriptResult>((resolve) => {
    ...
    const startClock = () => {
      ...
      timer = setTimeout(() => {
        finish({
          ok: false,
          error: `Script timed out (>${SCRIPT_TIMEOUT_MS / 1000}s of sandbox compute) and was aborted.`,
        });
      }, remainingMs);
    };
    startClock();
    ...
    hostPort.onmessage = (ev: MessageEvent) => {
      ...
      if (d?.t === 'done') {
        finish(
          d.ok
            ? { ok: true, result: d.result, logs: d.logs }
            : { ok: false, error: d.error, logs: d.logs },
        );
        return;
      }
      if (d?.t === 'rpc') {
        ...
        queue = queue.then(async () => {
          ...
          try {
            if (++rpcCount > RPC_LIMIT) {
              finish({
                ok: false,
                error: `Script exceeded the API call limit (${RPC_LIMIT}) and was aborted.`,
              });
              return;
            }
