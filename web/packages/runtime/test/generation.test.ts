...
test('reference roles are validated before submission, including one reference reused with an incompatible role',async t=>{
 const b=backend(),base=b.adapter.bind(b);b.adapter=(project='project')=>{const a=base(project);a.capture=async()=>({blob:new Blob(['video']),name:'clip.mp4',kind:'video',provenance:{kind:'asset'}});return a;};
 const f=fixture(b);t.after(()=>f.manager.dispose());const ref=ok(await f.manager.execute({action:'captureReference',source:{kind:'asset',assetId:'source'},expectedRevision:f.store.getState().revision})).reference;
 ok(f.store.dispatch({type:'updateGenerationSlot',slotId:f.slotId,request:{...request,firstFrameReferenceId:ref.id,referenceVideoIds:[ref.id]}}));const attempted=await f.manager.execute({action:'submit',slotId:f.slotId,intentVersion:2,requestId:'wrong-role'});assert.equal(attempted.ok,false);assert.match(attempted.message!,/media kind/);assert.equal(b.submits,0);
});
