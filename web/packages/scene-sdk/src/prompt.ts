...
export function scenePromptDoc(manifest: SceneAssetManifest): string {
  ...
  lines.push('Shared geometry: SceneSpec.geometries is a scene-local registry {geometryId:{name?,vertices,faces,uvs?}}. sceneEdit operations: {type:"geometry.create",id,geometry}, {type:"geometry.update",id,geometry} (replace the whole definition, updates all linked instances), {type:"geometry.remove",id} (reject while referenced). Add instances with {type:"add",kind:"prop",object:{id,model:"prop/instance",geometryId,position?,rotationX/Y/Z?,scale?,parentId?,color?,material?}}. Ordinary update/transform/layout/duplicate/remove work on each instance; duplicate keeps the shared reference. {type:"makeUnique",id} converts one instance into an independent prop/mesh, preserving its ID/transform/material and counting against the 200 ordinary-prop limit. It does not delete the shared resource. First version: opaque materials only; physics/bone attachment require makeUnique. Instances render in batches but remain independently pickable/editable.');
  lines.push('Budget and bulk editing: query({kind:"sceneBudget",assetId}) returns current counts/limits; query kind:sceneGeometries returns paginated geometry summaries (opt into fields:["id","geometry"] for vertices); query kind:sceneObjects includes geometryId. sceneEdit({assetId,edits,expectedRevision,preflight:true}) checks structure and budgets without compiling/rendering or committing; ready:false,compiled:false. A valid preflight is not proof of rendered quality. dryRun:true additionally compiles. includeSpec:false suppresses full specs in edit replies. Send at most 500 edits per batch; each committed batch is one undo step. Limits: ' + JSON.stringify(SCENE_LIMITS) + '. Exceeding a limit is a failed edit, never silently merge/simplify user-requested parts. Registry data is stored once in the native scene document; the 256 KiB scene budget and whole-scene compilation still apply.');
