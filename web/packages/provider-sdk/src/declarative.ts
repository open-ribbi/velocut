...
export interface HttpStep {
  ...
  response?:'json'|'bytes';
  error?:{path:string;success:JsonValue;message?:string};
}
export interface OutputMapping {kind:MediaKind;each?:string;url?:string;hex?:string;base64?:string;bytes?:boolean;mimeType?:string;durationS?:string}
export interface ModelSpec {
  ...
  execution:{
    ...
    poll? : HttpStep & {status:string;states:{running:string[];succeeded:string[];failed:string[]};receipt?:Record<string,string>;errorMessage?:string};
    collect?:HttpStep;
...
const stepProperties={method:{enum:['GET','POST','PUT','DELETE']},path:{type:'string',minLength:1},body:{},query:{type:'object'},headers:{type:'object',additionalProperties:{type:'string'}},response:{enum:['json','bytes']},error:object({path:{type:'string'},success:{},message:{type:'string'}},['path','success'])};
const receiptSchema={type:'object',additionalProperties:{type:'string'}};
const specValidator=ajv.compile(object({version:{const:1},id:{type:'string',pattern:'^[a-zA-Z0-9][a-zA-Z0-9_.:/-]*$'},label:{type:'string'},capability:{type:'string',minLength:1},inputSchema:{type:'object'},timeline:object({prompt:{type:'string'},duration:{type:'string'}}),execution:object({
  ...
  poll:object({...stepProperties,status:{type:'string'},states:object({running:{type:'array',items:{type:'string'}},succeeded:{type:'array',minItems:1,items:{type:'string'}},failed:{type:'array',items:{type:'string'}}},['running','succeeded','failed']),receipt:receiptSchema,errorMessage:{type:'string'}},['method','path','status','states']),
  collect:object(stepProperties,['method','path']),outputs:{type:'array',items:object({kind:{enum:['video','audio','image']},each:{type:'string'},url:{type:'string'},hex:{type:'string'},base64:{type:'string'},bytes:{const:true},mimeType:{type:'string'},durationS:{type:'string'}},['kind'])},data:{type:'string'},
...
export function parseModelSpec(source:string|unknown):ModelSpec {
  ...
  const checkStep=(s:HttpStep,phase:'submit'|'later')=>{
    ...
    if(s.error){parts(s.error.path);if(s.error.message)parts(s.error.message);}
  };
  checkStep(e.submit,'submit');if(e.poll){checkStep(e.poll,'later');parts(e.poll.status);if(e.poll.errorMessage)parts(e.poll.errorMessage);const states=Object.values(e.poll.states).flat();if(new Set(states).size!==states.length)throw Error('Task states must not overlap');}
  if(e.collect)checkStep(e.collect,'later');
  for(const mapping of [e.receipt,e.poll?.receipt])for(const [key,path] of Object.entries(mapping??{})){if(forbidden.has(key)||key.includes('.'))throw Error('Receipt field names must be simple keys');parts(path);}
  for(const o of e.outputs){if(['url','hex','base64','bytes'].filter(k=>Object.hasOwn(o,k)).length!==1)throw Error('Each output needs exactly one source');for(const k of ['url','hex','base64','each','durationS'] as const)if(o[k])parts(o[k]!);}
  if(e.data)parts(e.data);for(const path of Object.values(spec.timeline??{}))if(path)parts(path);
  return spec;
}
export const serializeModelSpec=(spec:ModelSpec)=>stringify(parseModelSpec(spec));
...
function readReceipt(mapping:Record<string,string>|undefined,response:unknown,prior:JsonObject={}):JsonObject {
  const result={...prior};for(const [key,path] of Object.entries(mapping??{})){const v=readPath(response,path);if(v===undefined)throw Error(`Response has no receipt field: ${path}`);safeData(v);result[key]=v as JsonValue;}return result;
}
function resultOf(spec:ModelSpec,response:unknown):ProviderResult {
  ...
  for(const o of spec.execution.outputs){
    const items=o.each?readPath(response,o.each):[response];if(!Array.isArray(items))throw Error('Output collection is not an array');
    for(const item of items){let source:ProviderResult['outputs'][number]['source'];
      if(o.url){const v=readPath(item,o.url);if(typeof v!=='string'||!['https:','http:'].includes(new URL(v).protocol))throw Error('Output URL is missing or invalid');source={kind:'url',url:v};}
      else if(o.bytes){if(!(item instanceof Uint8Array))throw Error('Expected binary response');source={kind:'bytes',bytes:item};}
      else {const v=readPath(item,(o.hex??o.base64)!);if(typeof v!=='string'||!v.length)throw Error('Encoded output is missing');let bytes:Uint8Array;
      ...
      const duration=o.durationS?readPath(item,o.durationS):undefined;if(duration!==undefined&&(typeof duration!=='number'||!Number.isFinite(duration)||duration<=0))throw Error('Invalid output duration');
      outputs.push({kind:o.kind,source,...(o.mimeType?{mimeType:o.mimeType}:{}),...(typeof duration==='number'?{durationS:duration}:{})});
    }
  }
  const data=spec.execution.data?readPath(response,spec.execution.data):undefined;if(spec.execution.data&&data===undefined)throw Error('Structured output is missing');if(data!==undefined)safeData(data);
  return {outputs,...(data!==undefined?{data:data as JsonValue}:{})};
}
...
export function declarativeProvider(source:ModelSpec):ProviderDefinition {
  ...
  return {id:'declarative:'+spec.id,label:spec.label??spec.id,apiVersion:1,capabilities:[spec.capability],credentials:e.auth?.type==='none'?[]:[{name:'apiKey',required:true}],configSchema:{type:'object'},create(context:ProviderContext){
    ...
    const send=async(step:HttpStep,env:Environment,phase:'execute'|'submit'|'poll'|'collect')=>{
      ...
      if(!response.ok)throw new ProviderError({phase,code:'MODEL_HTTP',message:`Model service HTTP ${response.status}`,httpStatus:response.status,outcome:response.status>=400&&response.status<500?'rejected':'unknown',retryable:(phase==='poll'||phase==='collect')&&(response.status>=500||response.status===429)});
      ...
      if(step.error&&JSON.stringify(readPath(json,step.error.path))!==JSON.stringify(step.error.success))throw new ProviderError({phase,code:'MODEL_REJECTED',message:step.error.message?String(readPath(json,step.error.message)??'Model rejected request'):'Model rejected request',outcome:'rejected',retryable:false});return json;
    };
    ...
    if(e.type==='sync-http')return {...common,execute:async(r,call)=>resultOf(spec,await send(e.submit,environment(call,modelInput(spec,r.input)),'execute'))};
    return {...common,
      ...
      poll:async(receipt,call)=>{const data=await send(e.poll!,environment(call,{},receipt),'poll'),status=readPath(data,e.poll!.status);if(typeof status!=='string')throw new ProviderError({phase:'poll',code:'UNKNOWN_STATUS',message:'Task status is missing',outcome:'unknown',retryable:false});
        if(e.poll!.states.failed.includes(status))return {state:'failed',error:{phase:'poll',code:'MODEL_FAILED',message:e.poll!.errorMessage?String(readPath(data,e.poll!.errorMessage)??'Model task failed'):'Model task failed',outcome:'rejected',retryable:false}};
        ...
        return {state:'succeeded',result:resultOf(spec,data)};
      },
      ...(e.collect?{collect:async(receipt:ProviderReceipt,call:ProviderCall)=>resultOf(spec,await send(e.collect!,environment(call,{},receipt),'collect'))}:{}),
    };
  }};
}
