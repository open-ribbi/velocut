...
const TOOLS: Anthropic.Tool[] = [
  ...
  },
  {
    ...
    description:
      ...
      '• await velocut.sceneSpatial({assetId,timeS?,expectedRevision?,queries}) reads anchors, raycasts, barycentric triangle samples, point distances and three-point angles at one scene/time. Results are world meters/degrees with revision, no full spec or renderer. Call sceneAssets or capabilities name:sceneSpatial for exact query schemas. sceneEdit anchor.set/anchor.remove authors object-local anchors. Use hit.local for a fixed local point, or hit.surfaceAnchor for a live attachment. Persist it with anchor.set, then compose binding.create/update/remove position/orientation relationships (full grammar in sceneAssets). Topology/source replacements invalidate attachments; vertex deformation follows automatically. Existing owned-field animation needs motion.referenceTimeS. Query sceneBindings for definitions and sceneSpatial type:bindings for evaluated status. These primitives do not auto-create assemblies or certify contact.\n' +
