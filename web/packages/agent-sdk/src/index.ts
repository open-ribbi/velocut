...
const TOOLS: Anthropic.Tool[] = [
  ...
  },
  {
    ...
    description:
      ...
      '• await velocut.motionClip({draw, build?, durationUs, atUs?, fps?, width?, height?, name?, images?}) → 用代码生成一段「动态图形」clip(标题卡/下三分之一条/动态字幕/信息图),自动落在「图形」视频轨。GSAP 是计时脊:在 build(tl,h) 里对你自己的对象建 PAUSED 时间轴(tl.to(obj,{opacity:1,y:0,duration:0.4,ease:"power2.out"},0)),velocut 每帧把 tl seek 到该帧再调 draw(ctx,t,h) 让你画;t 是秒。h 是绘制工具包,帮你绕开裸 canvas 的坑:h.text(str,{x,y,size,weight,color,align,baseline,maxWidth(自动测量折行,支持中文),lineHeight,stroke,strokeWidth,shadowColor,shadowBlur,opacity})、h.roundRect(x,y,w,h,r,{fill,stroke})、h.linearGradient/radialGradient、h.img(name)、h.lerp/map/clamp、h.rng(seed)(确定性随机,GSAP 随机没种子别用)、h.gsap、h.width/height/fps。draw 必须是 t 的纯函数(别读墙钟/Math.random/Date),velocut 已自动清屏+按资产分辨率渲染(无需管 dpr)+等字体就绪。坐标系=输出分辨率(默认整帧 w×h)。返回 {ok, assetId, clipId, trackId, frameCount}。例:做开场标题卡 velocut.motionClip({durationUs:2_500_000, atUs:0, build:(tl,h)=>{const s={a:0,y:40}; tl.to(s,{a:1,y:0,duration:0.5,ease:"power3.out"}).to(s,{a:0,duration:0.4},2.0); h._s=s;}, draw:(ctx,t,h)=>{const s=h._s; h.text("全职猎人",{x:h.width/2,y:h.height/2+s.y,size:96,weight:800,color:"#fff",align:"center",baseline:"middle",opacity:s.a,shadowColor:"rgba(0,0,0,.6)",shadowBlur:24});}})。把动态字幕跟旁白对齐:每句 tts 拿到 durationUs,就用同 atUs/durationUs 建一个 motionClip 字幕,入场/出场用 GSAP 缓动。\n' +
