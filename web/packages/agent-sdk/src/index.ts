...
const TOOLS: Anthropic.Tool[] = [
  ...
  },
  {
    ...
    description:
      ...
      '• await velocut.uploadFrame({timeUs, name?}) / await velocut.uploadClip({clipId, maxS?, name?}) → render the composite frame at an instant (PNG) / ONE clip in isolation (mp4, capped ~15s) and upload it to the user-configured store; returns {ok, handle: "upload://…", durationUs?}. Handles plug into videoGen reference fields: PREVIZ-TO-STYLED-RENDER, the flagship flow — const p = await velocut.uploadClip({clipId: sceneClipId}); then velocut.videoGen({prompt: "cinematic photoreal restyle of this blocking, golden hour", referenceVideoUrls: [p.handle], durationS: 5}) makes the provider follow the 3D previz\'s motion/composition while the prompt drives style. uploadFrame handles work as firstFrameUrl/lastFrameUrl (first/last-frame conditioning). Requires upload storage configured in Agent settings; if it errors as unconfigured, tell the user. Uploads publish the rendered media to the user\'s own store — mention it in your summary.\n' +
    ...
    input_schema: {
      ...
      properties: {
        code: {
          type: 'string',
          description: 'JavaScript source. Top-level await is allowed; use return to produce a JSON-serializable result. Globals: velocut (apply/tts/observe/motionClip/sceneClip/sceneAssets/videoGen/videoGenChannels/evaluate/document/seek) and console.',
        },
      },
      required: ['code'],
    },
