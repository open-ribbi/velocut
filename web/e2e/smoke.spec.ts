...
test('boots with an engine, imports media, edits, and survives a reload', async ({ page }) => {
  ...
  await page.locator('.asset-item').dragTo(page.locator('.timeline-panel canvas'), {
    targetPosition: { x: 200, y: 60 },
  });
  ...
  await page.reload();
  await expect
    .poll(async () => (await doc(page))?.tracks[0]?.clips.length, {
      timeout: 15_000,
    })
    .toBe(1);
...
test('projects are isolated: create, switch, delete via the switcher UI', async ({ page }) => {
  ...
  // Create a second project (name dialog) — the app reloads into it blank.
  ...
  // Delete the second project (confirmation dialog); it disappears from the list.
  await page.locator('.project-current').click();
  await page.locator('.project-row', { hasText: 'E2E Project' }).locator('.project-delete').click();
