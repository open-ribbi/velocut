...
import {VideoGenSettings} from './AgentConsole';
...
export function GenerationPanel({store,slotId,onSelect,onClose}:{store:Store;slotId:string|null;onSelect:(id:string|null)=>void;onClose:()=>void}){
  ...
  const need=duration*(selectedClip?.speed??1),planned=cap?.durationsS?[...cap.durationsS].sort((a,b)=>a-b).find(s=>s>=need):need;
  ...
  const save=()=>{
    ...
    const r=store.dispatch({type:'updateGenerationSlot',slotId:slot.id,startUs:Math.round(start*1e6),durationUs:Math.round(duration*1e6),request:draft});if(!r.ok)throw Error(r.error.message);
    const saved=store.getState().doc.generationSlots!.find(s=>s.id===slot.id)!;setBase(saved);return saved;
  };
  const capture=async(source:Record<string,unknown>)=>perform(async()=>{const r=await api({action:'captureReference',source,expectedRevision:store.getState().revision});setDraft(d=>({...d,firstFrameReferenceId:r.reference.id}));});
  return <Dialog open title={slot?'Generate video':'Generation jobs'} onClose={onClose} className="generation-dialog">
    ...
    {slot&&<>
      ...
      <label>Prompt<textarea aria-label="Video generation prompt" rows={3} value={draft.prompt} onChange={e=>setDraft({...draft,prompt:e.target.value})} placeholder="Describe the shot, action and camera motion…"/></label>
      <div className="generation-range"><label>Channel<select aria-label="Generation channel" value={draft.channel} onChange={e=>{const c=channels.find(c=>c.id===e.target.value);setDraft({...draft,channel:e.target.value,model:c?.defaultModel??c?.models[0]??''});}}><option value="">Choose channel</option>{channels.map(c=><option key={c.id} value={c.id}>{c.label??c.id}</option>)}</select></label><label>Model<select aria-label="Generation model" value={draft.model} onChange={e=>setDraft({...draft,model:e.target.value})}><option value="">Choose model</option>{channel?.models.map(m=><option key={m} value={m}>{m}</option>)}</select></label></div>
      <div className="generation-range"><label>Aspect ratio<input aria-label="Generation ratio" list="generation-ratios" value={draft.ratio??''} onChange={e=>setDraft({...draft,ratio:e.target.value||undefined})}/><datalist id="generation-ratios">{(cap?.ratios??['16:9','9:16','1:1']).map(r=><option key={r} value={r}/>)}</datalist></label><label>Resolution<input aria-label="Generation resolution" list="generation-resolutions" value={draft.resolution??''} placeholder="Provider default" onChange={e=>setDraft({...draft,resolution:e.target.value||undefined})}/><datalist id="generation-resolutions">{(cap?.resolutions??[]).map(r=><option key={r} value={r}/>)}</datalist></label></div>
      <label>Audio<select aria-label="Generated audio" value={draft.generateAudio===undefined?"default":String(draft.generateAudio)} onChange={e=>setDraft({...draft,generateAudio:e.target.value==="default"?undefined:e.target.value==="true"})}><option value="default">Provider default</option><option value="false">Off</option><option value="true" disabled={cap?.audio===false}>Generate audio</option></select></label>
      <details><summary>First-frame reference</summary>
        <label>Saved reference<select aria-label="Generation first frame" value={draft.firstFrameReferenceId??''} onChange={e=>setDraft({...draft,firstFrameReferenceId:e.target.value||undefined})}><option value="">Text only</option>{data.references.map(r=><option key={r.id} value={r.id}>{r.name}</option>)}</select></label>
        ...
        <label>Use imported image<select aria-label="Capture generation image" value="" disabled={busy||cap?.imageToVideo===false} onChange={e=>{if(e.target.value)void capture({kind:'asset',assetId:e.target.value});}}><option value="">Choose image to snapshot</option>{state.doc.assets.filter(a=>a.kind==='image'&&a.src.startsWith('opfs://')).map(a=><option key={a.id} value={a.id}>{a.name}</option>)}</select></label>
        <p className="empty-hint">A captured reference is stored with this project. Generate uploads that snapshot to your configured storage for the provider.</p>
      </details>
    ...
    {settings&&<><VideoGenSettings/><button onClick={()=>{setChannels(loadVideoGenConfig().channels);setSettings(false);}}>Done configuring channels</button></>}
  </Dialog>;
}
