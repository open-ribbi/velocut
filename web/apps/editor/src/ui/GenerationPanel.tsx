...
export function GenerationPanel({store,slotId,onSelect,onClose}:{store:Store;slotId:string|null;onSelect:(id:string|null)=>void;onClose:()=>void}){
  ...
  const capture=async(source:Record<string,unknown>)=>perform(async()=>{const r=await api({action:'captureReference',source,expectedRevision:store.getState().revision});setDraft(d=>{const kind=r.reference.kind??'image';if(kind==='image'&&preset?.references?.first!==false&&(!preset||preset.references?.first))return {...d,firstFrameReferenceId:r.reference.id};const key=kind==='video'?'referenceVideoIds':kind==='audio'?'referenceAudioIds':'referenceImageIds';return {...d,[key]:[...d[key]??[],r.reference.id]};});});
  return <Dialog open title={slot?'Generate video':'Generation jobs'} onClose={onClose} className="generation-dialog">
    ...
    {slot&&<>
      ...
      <details><summary>Reference media</summary>
        {(!preset||preset.references?.first)&&<label>Saved reference<select aria-label="Generation first frame" value={draft.firstFrameReferenceId??''} onChange={e=>setDraft({...draft,firstFrameReferenceId:e.target.value||undefined})}><option value="">Text only</option>{data.references.filter(r=>(r.kind??'image')==='image').map(r=><option key={r.id} value={r.id}>{r.name}</option>)}</select></label>}
