...
export function loadAudioChannels():ModelChannel[]{try{const x=JSON.parse(localStorage.getItem('velocut.audiogen')??'[]');return Array.isArray(x)?x.filter(c=>c&&typeof c.id==='string'&&AUDIO_PROTOCOLS.includes(c.kind)&&Array.isArray(c.models)):[];}catch{return [];}}
export function allModelChannels(){return [...loadVideoGenConfig().channels,...loadAudioChannels()];}
...
export async function generateConfiguredAudio(channelId:string,model:string,text:string,lyrics:string,parameters:Record<string,string|number|boolean>,signal:AbortSignal):Promise<File>{
