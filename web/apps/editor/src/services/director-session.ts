...
import { validateViewCamera, type SceneViewCamera, normalizeSceneSpec, sceneObjects, SCENE_VIEWS, type SceneView } from '@velocut/scene-sdk';
...
function createController(store: Store) {
  ...
  store.subscribe(() => {
    ...
    if (!asset?.spec) { state = null; notify(); return; }
    ...
    try {
      ...
      state = { ...state, objectId, focusId, timeS, playing: state.playing && timeS < spec.durationUs / 1e6,
        viewRevision: state.viewRevision + (focusId !== state.focusId ? 1 : 0) };
      notify();
    } catch { state = null; notify(); }
  });
  const update = (opts: DirectorSessionOptions) => {
    try {
      if (!opts || typeof opts !== 'object' || Array.isArray(opts)) throw new Error('director options must be an object');
      if (Object.keys(opts).some((k) => !['assetId', 'open', 'objectId', 'timeS', 'playing', 'view', 'mode', 'focusId', 'camera'].includes(k))) throw new Error('unknown director option');
      if (opts.open === false) { state = null; notify(); return { ok: true as const, state }; }
      const assetId = opts.assetId ?? state?.assetId;
      const asset = store.getState().doc.assets.find((a) => a.id === assetId);
      if (!asset?.src.startsWith('scene://') || !asset.spec) throw new Error('director requires a scene assetId');
      const spec = normalizeSceneSpec(JSON.parse(asset.spec));
      observedSpec = asset.spec;
      const fresh = !state || state.assetId !== asset.id;
      const current: DirectorSession = fresh ? { assetId: asset.id, objectId: null, timeS: 0, playing: false,
        view: 'perspective', mode: 'translate', focusId: null, viewRevision: 0 } : state!;
      const next = { ...current };
      for (const k of ['objectId', 'focusId'] as const) {
        if (opts[k] !== undefined) {
          if (opts[k] !== null && !sceneObjects(spec).some((e) => e.object.id === opts[k])) throw new Error(`unknown object '${opts[k]}'`);
          next[k] = opts[k]!;
        }
      }
      if (opts.timeS !== undefined) {
        if (!Number.isFinite(opts.timeS) || opts.timeS < 0 || opts.timeS > spec.durationUs / 1e6) throw new Error('timeS outside scene duration');
      ...
      if (opts.mode !== undefined) {
        if (!['translate', 'rotate', 'scale'].includes(opts.mode)) throw new Error('unknown gizmo mode');
        next.mode = opts.mode;
      }
      if (opts.view !== undefined || opts.focusId !== undefined) delete next.camera;
      if (opts.camera != null) {
        const err = validateViewCamera(opts.camera); if (err) throw new Error(err);
      ...
      if (opts.view !== undefined || opts.focusId !== undefined || opts.camera !== undefined) next.viewRevision++;
      state = next; notify();
      return { ok: true as const, state };
    } catch (e) { return { ok: false as const, message: e instanceof Error ? e.message : String(e) }; }
  };
  return { getSnapshot: () => state, update,
    subscribe: (fn: () => void) => { listeners.add(fn); return () => { listeners.delete(fn); }; } };
}
export function directorController(store: Store) {
  let controller = controllers.get(store);
  if (!controller) { controller = createController(store); controllers.set(store, controller); }
  return controller;
}
