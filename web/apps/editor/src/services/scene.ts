...
import { MAX_MODEL_BYTES, validateGlb, loadImportedModel, nextSceneId, type SceneModel, sceneObjects, normalizeSceneSpec, applySceneEdits, type SceneEdit, type SceneViewCamera, type SceneView, compileSceneSpec, validateSceneSpec, type SceneSpec, type CompiledScene } from '@velocut/scene-sdk';
...
import { sceneResources, modelDigest } from './scene-resources';
...
async function compileFor(store: Store, spec: SceneSpec, doc: VDocument = store.getState().doc): Promise<CompiledScene> {
  ...
  try { await compiled.load(); return compiled; }
  catch (e) { compiled.dispose(); throw e; }
}
 ...
 */
export async function createSceneClip(store: Store, media: MediaLibrary, opts: SceneClipOptions, dispatch: SceneDispatch = (cmd) => store.dispatch(cmd)): Promise<SceneResult> {
  let spec = opts?.spec;
...
export function bindSceneAuthoring(store: Store, media: MediaLibrary): void { authoringMedia.set(store, media); }
...
export async function replaceSceneSpec(store: Store, assetId: string, spec: SceneSpec,
  expectedRevision: number, dispatch: SceneDispatch = (cmd) => store.dispatch(cmd)) {
  ...
  try {
    ...
    if (before.revision !== expectedRevision) throw new Error('conflict: document changed; read the scene again');
    ...
    if (new TextEncoder().encode(serialized).length > 262144) throw new Error('scene spec exceeds 256 KiB');
    compiled = await compileFor(store, normalized);
    if (store.getState().revision !== expectedRevision ||
    ...
    attach(media, assetId, compiled);
    attachedSpecs.set(assetId, serialized);
    compiled = undefined; // ownership transferred
    return { ok: true as const, assetId, revision: store.getState().revision, ready: true, spec: normalized };
  } catch (e) {
    compiled?.dispose();
...
export async function editScene(store: Store, opts: SceneEditOptions, dispatch?: SceneDispatch) {
  try {
    ...
    const committed = await replaceSceneSpec(store, opts.assetId, result.spec, opts.expectedRevision ?? before.revision, dispatch);
...
export async function inspectScene(store: Store, opts: { assetId: string; timeS?: number; view?: SceneView; objectId?: string; camera?: SceneViewCamera }, capture = false) {
  let compiled: CompiledScene | undefined;
  try {
    const { spec, revision } = readScene(store, opts.assetId);
    const doc = store.getState().doc;
    const width = spec.width ?? doc.width, height = spec.height ?? doc.height;
    const factor = Math.min(1, 768 / Math.max(width, height));
    compiled = await compileFor(store, { ...spec, width: Math.max(16, Math.round(width * factor)), height: Math.max(16, Math.round(height * factor)) });
...
export async function arrangeScene(store: Store, opts: SceneArrangeOptions, dispatch?: SceneDispatch) {
  try {
    if (!Array.isArray(opts.ids) || !opts.ids.length || opts.ids.length > 200 || new Set(opts.ids).size !== opts.ids.length) throw new Error('ids must contain 1..200 unique object IDs');
    if (!['ground', 'on', 'align'].includes(opts.mode)) throw new Error('unknown arrange mode');
    if (opts.axis != null && !['x', 'y', 'z'].includes(opts.axis)) throw new Error('unknown align axis');
    if (opts.edge != null && !['min', 'max', 'center'].includes(opts.edge)) throw new Error('unknown align edge');
    const gap = opts.gap ?? 0;
    if (!Number.isFinite(gap)) throw new Error('gap must be finite meters');
    const r = await inspectScene(store, opts);
    if (!r.ok) return r;
    if (opts.expectedRevision != null && opts.expectedRevision !== r.revision) throw new Error('conflict: document changed; read the scene again');
    const targets = new Map(r.objects.map((o) => [o.id, o]));
    const reference = opts.referenceId ? targets.get(opts.referenceId) : undefined;
    if (opts.mode !== 'ground' && !reference?.bounds) throw new Error('on/align requires a referenceId with geometry');
    ...
    for (const id of [...opts.ids, ...(reference ? [reference.id] : [])]) {
      ...
      while (parent) {
        if (moves.has(id) && parent === reference?.id) throw new Error('reference includes a moving object; choose a separate support object');
        if (moves.has(parent)) throw new Error('cannot arrange an object together with its descendant or use its descendant as reference');
    ...
    for (const id of opts.ids) {
      ...
      if ('physics' in authored && authored.physics && (opts.timeS ?? 0) !== 0) throw new Error('arrange physics props at timeS:0 to change their initial pose');
      const delta = new T.Vector3();
      if (opts.mode === 'align') {
        const axis = opts.axis ?? 'x', i = { x: 0, y: 1, z: 2 }[axis], edge = opts.edge ?? 'center';
        delta[axis] = reference!.bounds![edge][i] - object.bounds[edge][i] + gap;
      } else {
        delta.y = (opts.mode === 'ground' ? 0 : reference!.bounds!.max[1]) + gap - object.bounds.min[1];
        if (opts.mode === 'on') {
      ...
      delta.applyMatrix4(inverse).sub(new T.Vector3().applyMatrix4(inverse)).divideScalar(object.positionScale);
      const position = { ...authored.position };
      for (const axis of ['x', 'y', 'z'] as const) {
        if (Math.abs(delta[axis]) < 1e-9) continue;
        const fallback = object.kind === 'prop' && !('attachTo' in authored && authored.attachTo) && axis === 'y' ? 0.5 : 0;
        const previous = position[axis] ?? fallback;
        position[axis] = Array.isArray(previous) ? previous.map((k) => ({ ...k, v: k.v + delta[axis] })) : previous + delta[axis];
      }
      edits.push({ type: 'update', id, patch: { position } });
    }
    return editScene(store, { assetId: opts.assetId, edits, expectedRevision: r.revision }, dispatch);
  } catch (e) { return { ok: false as const, message: e instanceof Error ? e.message : String(e) }; }
}
...
export async function importSceneModel(store: Store, opts: SceneModelImportOptions, dispatch?: SceneDispatch) {
  try {
    const before = readScene(store, opts.assetId);
    const expected = opts.expectedRevision ?? before.revision;
    if (expected !== before.revision) throw new Error('conflict: document changed; read the scene again');
    if (opts.kind != null && opts.kind !== 'prop' && opts.kind !== 'character') throw new Error('import kind must be prop or character');
    ...
    if (opts.file instanceof File) {
      ...
      if (typeof opts.base64 !== 'string' || opts.base64.length > Math.ceil(MAX_MODEL_BYTES / 3) * 4) throw new Error('provide a GLB file or base64 payload (up to 64 MiB)');
    ...
    const src = await saveMedia(new File([bytes], `scene-model-${hash}.glb`, { type: 'model/gltf-binary' }), activeStorage().mediaDir);
    ...
    for (const clip of model.animations) {
      if (!clip.name || Object.hasOwn(clips, clip.name)) throw new Error('GLB animation names must be non-empty and unique');
      Object.defineProperty(clips, clip.name, { value: { loop: true }, enumerable: true, writable: true, configurable: true });
    }
    ...
    (before.spec.models ??= {})[modelId] = { src, label, heightM: bounds.max.y - bounds.min.y, clips, bones, morphs: [...morphs] };
    if (opts.kind === 'character') {
      const first = Object.keys(clips)[0];
      (before.spec.characters ??= []).push({ id: objectId, model: modelId, name: label,
        ...(first ? { actions: [{ clip: first, start: 0 }] } : {}) });
    } else (before.spec.props ??= []).push({ id: objectId, model: modelId, name: label, position: { y: 0 } });
...
export function dispatchSceneAware(store: Store, cmd: Command, dispatch: SceneDispatch = (c) => store.dispatch(c)): Envelope | Promise<Envelope> {
  const touchesScene = (c: Command): boolean => {
    ...
    if (c?.type === 'setAssetSpec') return store.getState().doc.assets.some((a) => a.id === c.assetId && a.src.startsWith('scene://'));
    return false;
  };
  ...
  return (async () => {
    ...
    try {
      ...
      if (!valid.ok) return { ok: false as const, error: { code: valid.code, message: valid.message } };
      ...
      for (const asset of candidate.assets) {
        ...
        if (!asset.spec) throw new Error('scene assets require a spec; remove the asset to delete a scene');
        const spec = JSON.parse(asset.spec);
        const error = validateSceneSpec(spec);
        if (error) throw new Error(error);
        if (Math.round(spec.durationUs) !== asset.durationUs) throw new Error('scene duration must agree with its asset metadata');
        prepared.push({ asset, compiled: await compileFor(store, spec, candidate) });
      }
      if (store.getState().revision !== before.revision) throw new Error('conflict: document changed while compiling the batch');
      const result = dispatch(cmd);
      if (!result.ok) return result;
      for (const { asset, compiled } of prepared) {
        attach(media, asset.id, compiled); attachedSpecs.set(asset.id, asset.spec!);
      }
      prepared.length = 0;
      return result;
    } catch (e) { return { ok: false as const, error: { code: 'invalidScene', message: e instanceof Error ? e.message : String(e) } }; }
    finally { for (const p of prepared) p.compiled.dispose(); }
  })();
}
