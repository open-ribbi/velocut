...
共享几何首轮：场景内 `geometries` 注册表和 `prop/instance` 引用已实现，实例数量上限已取消（预算字段为 null）；同几何／材质合批绘制，逻辑对象仍可独立选择、查询、变换、复制和转独立网格。`sceneEdit` 新增纯预算预检，`query` 新增几何分页及预算查询。后续已接入不可变二进制几何资源、分段读取／索引修改，以及实例／分组变换的增量渲染更新。场景文本字节上限已取消，几何另有独立预算。拓扑／材质／物理编辑仍完整编译，万实例规模、显隐／缩放动画和视频任务交付尚未实现。接口与边界见[使用说明](../integrations/atomic-api.md#shared-native-geometry-and-instances)。
