...
已新增独立 resource.import、media.probe 任务和事务内 registerAsset，以及 Rust/TS 双引擎 duplicateClip。任务可查询、取消，结果不会自动登记或落轨；登记与插入仍由 Agent 显式组合。文件存储持久，任务／句柄只保留在运行实例内。具体限制和当前接口见使用说明。下方目录仍保留设计基线，具体实现状态以使用说明与能力发现为准。

**共享几何首轮进度**

共享几何首轮：场景内 `geometries` 注册表和 `prop/instance` 引用已实现，单场景支持最多 1000 个实例；同几何／材质合批绘制，逻辑对象仍可独立选择、查询、变换、复制和转独立网格。`sceneEdit` 新增纯预算预检，`query` 新增几何分页及预算查询。后续已接入不可变二进制几何资源、分段读取／索引修改，以及实例／分组变换的增量渲染更新。256 KiB 约束精简场景描述，几何另有 16 MiB 预算。拓扑／材质／物理编辑仍完整编译，万实例规模、显隐／缩放动画和视频任务交付尚未实现。接口与边界见[使用说明](../integrations/atomic-api.md#shared-native-geometry-and-instances)。

最新增量已实现历史 spec 字符串共享与紧凑保存、`geometry.clone` 与资源化 `makeUnique`，以及共享材质的创建／更新／删除和 `materialId` 绑定。保留所有撤销分支与旧工程读取；历史旧格式保存值保留为恢复副本。实例数量上限、动画资源复用、项目工具与视频任务交付不因此自动扩展。
