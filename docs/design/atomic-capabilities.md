...
共享几何首轮：场景内 `geometries` 注册表和 `prop/instance` 引用已实现，单场景支持最多 1000 个实例；同几何／材质合批绘制，逻辑对象仍可独立选择、查询、变换、复制和转独立网格。`sceneEdit` 新增纯预算预检，`query` 新增几何分页及预算查询。当前仍是 256 KiB 场景文档和全场景编译；二进制几何资源、增量编译、万实例规模、显隐／缩放动画和视频任务交付不在本轮实现中。接口与边界见[使用说明](../integrations/atomic-api.md#shared-native-geometry-and-instances)。
