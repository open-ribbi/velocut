...
fn run_vector(path: &PathBuf) {
    ...
    let expect = &v["expect"];
    if let Some(slots) = expect.get("generationSlots") {
        let doc: Value = serde_json::from_str(&engine.document_json()).unwrap();
        assert_data(&doc["generationSlots"], slots);
    }
    ...
    if let Some(evals) = expect.get("eval").and_then(|e| e.as_array()) {
        for case in evals {
            ...
            if let Some(ids) = case.get("pendingGenerationIds") {
                assert_data(
                    fg.get("pendingGenerationIds")
