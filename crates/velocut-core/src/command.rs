...
pub enum Event {
    ...
    #[serde(rename_all = "camelCase")]
    GenerationSlotUpdated {
        slot_id: String,
    },
    #[serde(rename_all = "camelCase")]
    GenerationSlotRemoved {
        slot_id: String,
    },
    #[serde(rename_all = "camelCase")]
    AssetAdded {
...
pub(crate) fn apply_inner(doc: &mut Document, cmd: &EditCommand) -> CmdResult {
    if let Some(result) = crate::generation::apply_generation(doc, cmd) {
        return result;
    }
    use EditCommand::*;
    match cmd {
        EditCommand::AddGenerationSlot { .. }
        | EditCommand::UpdateGenerationSlot { .. }
        | EditCommand::RemoveGenerationSlot { .. }
        | EditCommand::ResolveGenerationSlot { .. }
        | EditCommand::ReplaceClipSource { .. } => unreachable!(),
        AddAsset {
            kind,
            src,
            name,
